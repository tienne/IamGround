...
gulp.task('resources.Assets', () => {
    return gulp.src([`${SRC}**/*`, `!${SRC}app/`, `!${SRC}test/`, '!**/*.spec.*', '!**/*.js', '!**/*.ts', '!**/*.sass', '!**/*.html'], {follow: true})
...
gulp.task('project.Styles', () => {
    return gulp.src([`${SRC}**/*.sass`, '!**/*.tns.*'], {follow: true})
        .pipe(gulp.dest(DEST));
});
...
gulp.task('tns.Styles', () => {
    return gulp.src([`${SRC}**/*.tns.sass`, `${SRC}**/*.tns.ios.sass`, `${SRC}**/*.tns.android.sass`], {follow: true})
        .pipe(rename(removeTns))
...
gulp.task('phone.Styles', () => {
    return gulp.src([`${SRC}**/*.tns.phone.sass`, `${SRC}**/*.tns.ios.phone.sass`, `${SRC}**/*.tns.android.phone.sass`], {follow: true})
        .pipe(rename(removeTns))
        .pipe(rename(removePhone))
...
/**
 * For non webpack builds, sass needs to be converted to css
 */
...
gulp.task('tns.Livesync', () => {
    return gulp.watch([`${SRC}**/*.tns.html`, `${SRC}/**/*.tns.sass`, `${SRC}/**/*.component.ts`])
        .on('change', (file) => {
...
gulp.task('tns.Livesync.Phone', () => {
    return gulp.watch([`${SRC}**/*.tns.phone.html`, `${SRC}/**/*.tns.phone.sass`, `${SRC}/**/*.component.ts`])
        .on('change', (file) => {
            var outputDest = file.replace(SRC, DEST);
            outputDest = outputDest.substring(0, outputDest.lastIndexOf('/'));
            gulp.src([file])
